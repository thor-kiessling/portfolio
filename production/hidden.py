...
def extract_data(filename):
    ...
    for line in open(filename):
        row = line.split(",")
        averagenumerator = sum(float(x) for x in row[0:3])
        if averagenumerator/3 > float(row[3]):
            labels.append(1)
        else:
            labels.append(0)

        fvecs.append([float(x) for x in row[3:]])
...
def main(argv=None):
    ...
    with tf.Session() as s:
        ...
        for step in range(num_epochs * train_size // BATCH_SIZE):
            if verbose:
                print(step, "Accuracy:", accuracy.eval(feed_dict={x: test_data, y_: test_labels}))
