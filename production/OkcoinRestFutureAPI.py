...
class OKCoinFuture:
    ...
    # get history written by thorb
    # def future_kline(self, symbol=None, type=None, contract_type=None, size=None, since=None):
    ...
    #     if size:  # dunno lol
    #         params += '&size=' + size if params else 'size=' + size
    #     if since:  # timestamp, maybe utc? 13 digits in example, 10 from OS
    #         params += '&since=' + since if params else 'since=' + since
    #     return httpGet(self.__url, FUTURE_KLINE, params)

    def future_kline(self, symbol='btc_usd', type='1min', size=1, contract_type="quarter", since=None):
