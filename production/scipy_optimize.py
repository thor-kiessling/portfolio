...
def main():
    data = extract_data()
    minutes_train = 400000
    # minutes_train = 1300000  # four zeros = 1 week, five zeroes = 10 weeks,  1300000 = all my data 500000=1year
    #bounds = [(-0, 0), (2, 1000), (-0, 0), (40, 40000), (-1, 1), (10, 40000), (.0001, .002), (10, 40000)]
    #                                                                                atr reverse trigger, lookback 6/7
    #                      max            max        max         max
    bounds = [ (2, 3000), (2, 24000), (0.0, .04), (1, 80), (100, 50000)]
    ...
    # objective([475.0, 1285.0, 0.0018392239365409187, 17.806995940901789, 1670.7339655391988], data=data, size=minutes_train, train=True, plot=True) # old running
    ...
    # objective([1010.0, 9988.0, 0.016877194724765399, 38.140250881983718, 9958.2905700554747], data=data, size=minutes_train, train=True, plot=True) # current runninig
    # test_result = objective([1010.0, 9988.0, 0.016877194724765399, 38.140250881983718, 9958.2905700554747], data=data, size=minutes_train, train=False, plot=True)
    objective([856.0, 10959.0, 0.015403980409811828, 71.942292962805908, 13542.830248688084], data=data, size=minutes_train, train=True, plot=True) # new candidate w/rules
    test_result = objective([856.0, 10959.0, 0.015403980409811828, 71.942292962805908, 13542.830248688084], data=data, size=minutes_train, train=False, plot=True)
    # objective([730.0, 9874.0, 0.018242839616007783, 29.616646473123861, 9789.4918810651106], data=data, size=minutes_train, train=True, plot=True) # cand3
    # test_result = objective([730.0, 9874.0, 0.018242839616007783, 29.616646473123861, 9789.4918810651106], data=data, size=minutes_train, train=False, plot=True)
