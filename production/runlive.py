...
def fut_on_open(self):
    futapikey = api_keys.ok_apikey
    futsecretkey = api_keys.ok_secretkey
...
def fut_on_message(self, evt):  # should not ever block, will be many many calls
    ...
    if isinstance(data, bool):
        notification("settlement", "sleeping for 30")
        time.sleep(30)
        return
        ...
        for msg in data:
            ...
            if 'data' in msg:
                if 'result' in msg['data']:
                    if msg['data']['result'] == False or msg['data']['result'] == 'false':
            ...
            if str(msg['channel']) == 'ok_sub_futureusd_userinfo':
                ...
                if int(msg['data']['status']) == 0 or int(msg['data']['status']) == 1:  # order is active on books
                    self.namespace.trades_are_temp_flag = False
...
def future_get_orders(ns, api_key, secretkey, symbol, contractType, orderId=None):
    ...
    sign = buildMySign(params, secretkey)
    finalStr = "{'event':'addChannel','channel':'ok_futureusd_orderinfo','parameters':{'api_key':'" + api_key + \
               "','sign':'" + sign + "','symbol':'" + symbol + "','order_id':'" + orderId + "','contract_type':'" + \
               contractType + "','status':'" + status + "','current_page':'" + current_page + "','page_length':'" + page_length + "'}}"
    # websocket.send( this is directly from the API docs, why do I get 20100: request timed out?
    #     "{'event':'addChannel','channel':'ok_futureusd_orderinfo','parameters': {'api_key':'XXXX','sign':'XXXX','symbol'"
    #     ":'ltc_usd','order_id':'20914907', 'contract_type':'this_week','status':'1','current_page':'1','page_length':'1'}}");
    while glob_futws.sock is None or not glob_futws.sock.connected:
        time.sleep(1)
    return finalStr
...
def desired_position(ns):
    ...
    if ns.direction == 'short': # normal enter new short
        desired_shorts = max_contracts
        desired_longs = 0
        emergency_mode = False
    ...
    elif ns.direction == 'long':  # normal enter new long
        desired_longs = max_contracts
        desired_shorts = 0
        emergency_mode = False
    ...
    if ns.reversal_flag :
        emergency_mode = False  # todo someday code the bit where it trades with prejudice market goes real bad
    # else:
    #     desired_longs = 0
    #     desired_shorts = 0
...
def trade(longs_delta, shorts_delta, emergency_mode, ns):
    ...
    if trades != {} or longs_delta != 0 or shorts_delta != 0:
        ...
        #print("                             ", possibilities)
    pennysnipe_bid = str(float(closest_bid[0]) + .02)
    pennysnipe_ask = str(float(closest_ask[0]) - .02)
    ...
    for ordertype in possibilities:  # ordertype[2] % 2 == 0
        ...
        if ordertype[0] > 0 or ordertype[4] > 0 and glob_futws.sock.connected:
            ...
            if closest_ask[0] - closest_bid[0] < .05 and not emergency_mode: # and not len(trades) >= 1:  # market buy that which is possible
                ...
                while glob_futws.sock is None or not glob_futws.sock.connected:
                    time.sleep(1)
            elif not emergency_mode and ordertype[0] - ordertype[4] > 0: # and not len(trades) >= 1:
                ...
                #ns.book_contracts += ordertype[0] - ordertype[4] old style book_contracts number
                if ordertype[1] == 1:
                    ns.book_contracts_open_long += ordertype[0] - ordertype[4]
                elif ordertype[1] == 2:
                    ns.book_contracts_open_short += ordertype[0] - ordertype[4]
                elif ordertype[1] == 3:
                    ns.book_contracts_close_long += ordertype[0] - ordertype[4]
                elif ordertype[1] == 4:
                    ns.book_contracts_close_short += ordertype[0] - ordertype[4]
                ns.trades_are_temp_flag = True
            ...
            for name, listing in trades.items():
                ...
                if int(listing['type']) == ordertype[3] and ordertype[0] > 0:  # cancel existing open ~opposite~ orders
                    glob_futws.send(
                        future_cancel_order(ns, futapikey, futsecretkey, symbol, str(listing['order_id']),
                                            contractType))
                    if int(listing['type']) == 1:
                        ns.book_contracts_open_long -= int(listing['amount']) - int(listing['deal_amount'])
                    elif int(listing['type']) == 2:
                        ns.book_contracts_open_short -= int(listing['amount']) - int(listing['deal_amount'])
                    elif int(listing['type']) == 3:
                        ns.book_contracts_close_long -= int(listing['amount']) - int(listing['deal_amount'])
                    elif int(listing['type']) == 4:
                        ns.book_contracts_close_short -= int(listing['amount']) - int(listing['deal_amount'])
                    ns.trades_are_temp_flag = True
...
def evolved_predictor(ns, previous_minute, data):
    ...
    # old [1010.0, 9988.0, 0.016877194724765399, 38.140250881983718, 9958.2905700554747]
    # [856.0, 10959.0, 0.015403980409811828, 71.942292962805908, 13542.830248688084]
    ...
    ema1 = ta.EMA(truncated_data, timeperiod=856)
    #ns.stddev = max(np.std(truncated_data[-12554:]) / truncated_data[-1], .0001)
    ema2 = ta.EMA(truncated_data, timeperiod=10959)
    # ns.ema2 = ema2
    ...
    #     ns.reversal_flag = False
    #     pickle.dump(ns.reversal_flag, open('reversal_flag', 'wb'))
    low_threshold = ema2 * (1 - (.01540398*(71.94-(min(minutes_since_last_trade, 13542)/(13542/(71.94-1))))))
    high_threshold = ema2 * (1 + (.01540398*(71.94-(min(minutes_since_last_trade, 13542)/(13542/(71.94-1))))))

    use_stoploss = False
    if data[-2] < low_threshold and ns.direction == "long":
        use_stoploss = True
        print("Triggered low threshold stoploss")
    if data[-2] > high_threshold and ns.direction == "short":
        use_stoploss = True
        print("Triggered high threshold stoploss")

    if use_stoploss:  # first check if we hit stoploss
    ...
    else:  # always called during non-stoploss, vast majority
        if ns.reversal_flag:  # check to see if ema agrees with stoploss, remove reverse flag if true, don't trade
            if ema1 > ema2 and ns.dyn_ema == 1:
                ns.reversal_flag = False
                pickle.dump(ns.reversal_flag, open('reversal_flag', 'wb'))
            if ema1 < ema2 and ns.dyn_ema == -1:
                ns.reversal_flag = False
                pickle.dump(ns.reversal_flag, open('reversal_flag', 'wb'))
            # todo put something in here for setting dyn_ema during re-loading program while in stoploss state
        else:  # normal following ema stuff
            if ema1 > ema2:
                ns.dyn_ema = 1
            else:
                ns.dyn_ema = -1
    ...
    if previous_minute != datetime.now().minute:
        print("current price:", ns.glob_future_price, "ema1 ema2", round(ema1, 2), round(ema2, 2), datetime.now())
        if ns.dyn_ema > 0:
            print("  next stoploss when crossover", round(low_threshold, 2), "reverse:", ns.reversal_flag)
        else:
            print("  next stoploss when crossover", round(high_threshold, 2), "reverse:", ns.reversal_flag)
...
def dyn_mash_and_predict(ns):
    ...
    while True:
        ...
        p = Process(target=evolved_predictor, args=(ns, previous_minute, data[-10960:]))#, args=(now.minute, ), name=str(now.minute))
        p.daemon = True
        p.start()
...
def main(ns):
    ...
    while ns.dyn_ema == 0.0:
        time.sleep(1)
        print("waiting on dyn_ema")
    orderbook = copy.deepcopy(ns.glob_orderbook)
    while orderbook == {}:
        time.sleep(1)
        print("Waiting on orderbook")
        orderbook = copy.deepcopy(ns.glob_orderbook)
    ...
    try:
        while True:
            ...
            while glob_futws.sock is None or not glob_futws.sock.connected:
                time.sleep(1)
            if not ns.trades_are_temp_flag:
                ns.book_contracts_close_long = 0
                ns.book_contracts_close_short = 0
                ns.book_contracts_open_long = 0
                ns.book_contracts_open_short = 0
