...
def objective(variables, data=None, size=None, train=None, plot=None):
    ...
    num_samples = variables[1]
    num_samples = int(num_samples)
    predictions_to_avg = variables[0]
    predictions_to_avg = int(predictions_to_avg)
    ...
    data = data[-1 * size - 614:]
    data = np.array(data)
    data2 = []
    minutes_look_ahead = 1
    for index, line in enumerate(data):
        temp_list = np.zeros(look_back_window)
        if index < look_back_window: continue
        #if index > len(data) - minutes_look_ahead - 1: continue
        for i in range(look_back_window):
            temp_list[i] = data[index - look_back_window + i][4]  # properly grabs up to index-1
        mean = temp_list.mean(axis=0)
        data2.append((line - mean) / line[4])
        if index % 1000 == 0:
            print(index / len(data))
    data2 = np.array(data2, dtype=np.float32)
    data2 *= 19.5874844188  # trained value to normalize amplitude
    ...
    for iteration in range(size):
        ...
        try:
            ...
            temp_list = np.zeros(look_back_window)
            for i in range(look_back_window):
                temp_list[i] = data[-1 * size + iteration - look_back_window + i][4]  # properly grabs up to index-1
            ma100 = temp_list.mean(axis=0)
            truncated_data = data2[-1 * size + iteration - 614:-1 * size + iteration:, 4]  # numpy array view
            predictions = wavenet_generate.main(truncated_data, num_predictions=num_samples) # takes about 40s each with "fast"gen
            # print(len(predictions))                                                       # takes 8s with slowgen
            predictions = predictions[-num_samples:]
            prediction_mean = np.mean(predictions)
            prediction_mean /= 19.5874844188
            # prediction_mean *= ma100  # todo maybe switch these
            # prediction_mean += line[4]
            prediction_mean *= line[4]  # todo maybe switch these
            prediction_mean += ma100
            local_ma100_history.append(ma100)
            # print(predictions)

            prediction_history = np.append(prediction_history, prediction_mean)

            prediction = np.mean(prediction_history[max(-predictions_to_avg, -len(prediction_history)):])

            # print(truncated_data[-1])
            # print(predictions, len(predictions)) #todo debug
            # print(prediction)
        ...
        if prediction > line[4]:
        #if plusdi[-1] > minusdi[-1]:
            if plot:
                print("prediction", prediction, "is higher than current", line[4], "and", (prediction - line[4])/line[4] * 100, "different from lastprice")

            buy += 1
        else:
            if plot:
                print("prediction", prediction, "is lower  than current", line[4], "and", (prediction - line[4])/line[4] * 100, "different from lastprice")
        ...
        if not long and (buy_sell > 0.0): # main buy/sell
            ...
            if plot:
                ...
                print("entered long, last trade", return_graph_y[-1] / return_graph_y[-2])
            local_return_graph_x.append(line[0])
            local_return_graph_y.append(returns)

            continue

        if long and (buy_sell < 0.0):  #  main buy/sell 2
            #                      on 20x
            ...
            if plot:
               ...
               #global short_enter_y
                ...
                return_graph_x.append(line[0])
                return_graph_y.append(returns)
                print("entered short, last trade", return_graph_y[-1] / return_graph_y[-2])
            local_return_graph_x.append(line[0])
            local_return_graph_y.append(returns)
        ...
        if returns < 50 and not plot:  # fail-quick
            ...
            break
    ema1_switches = 0
    for index, item in enumerate(prediction_history):
        if (item > prediction_history[index-1]) is not (prediction_history[index-1] > prediction_history[index-2]):
            ema1_switches += 1
    macd_switches = 0
    ...
    print("average trade mean + median:", np.mean(buyselllist), np.median(buyselllist))
    print("best trade:", max(buyselllist), "worst:", min(buyselllist))
    if plot:
    ...
    if plot:
        print (ema1_switches, "trade signals:", macd_switches, "MDD%", max_draw_down * 100, "dd_rel%:", mdd_relative * 100, "max leverage:", max_leverage)
    else:
        print(ema1_switches,  "trade signals:", macd_switches, "MDD%", max_draw_down * 100, "dd_rel%:",
              mdd_relative * 100)
    ...
    if plot:
        ...
        global plot_plusdi
        if train:
            plt.plot(data[int(-len(prediction_history)-(size/10)):int(-(size/10)), 0], prediction_history, "c-")
            plt.plot(data[int(-len(local_ma100_history) - (size / 10)):int(-(size / 10)), 0], local_ma100_history, "b-")
            #print(prediction_history)
            ##global plot_adx


        else:
            plt.plot(data[-len(prediction_history):, 0], prediction_history, "c-")
            plt.plot(data[-len(local_ma100_history):, 0], local_ma100_history, "b-")
...
def main():
    data = extract_data()
    minutes_train = 1000  # four zeros = 1 week, five zeroes = 10 weeks,  1100000 = all my data 500000=1year
    ...
    #objective([1, 5], data=data, size=minutes_train, train=True, plot=True)
    #test_result = objective([1, 5], data=data, size=minutes_train, train=False, plot=True)
    ...
    ax2.plot(return_graph_x, return_graph_y, "k-")
    ax2.set_yscale('log')
