...
void error(int id)
{
    ...
    else if ( id == 4 )
    {
        ...
        printf("Command Usage:  UCR_DTW.exe  data-file  query-file   m   R     n\n\n");
        printf("For example  :  UCR_DTW.exe  data.txt   query.txt   128  0.05  1\n");
    }
    exit(1);
}
...
int main(  int argc , char *argv[] )
{
    ...
    /// read number of results returned
    int n = atoi(argv[5]);
    ...
    // print last n locs
    std::cout << std::endl << n << " last locs:" << std::endl;
    auto start = list.end() - n;
    if (n > list.size()) // if n > list length print everything we got
        start = list.begin();
    decltype(list) last_n(start, list.end());
    for (auto &x : last_n) {
        std::cout << x << std::endl;
    }
    std::cout << std::endl;
