...
def extract_dyn_data():
    ...
    for index, line in enumerate(data):
        temp_list = []
        if index < len(data) - num_samples - minutes_look_ahead - look_back_window: continue  # TODO temp - eliminates the first 100000
        ...
        for i in range(look_back_window):
            temp_list.append(data[index-look_back_window + i])  # properly grabs up to index-1
    ...
    fvecs_abs = np.absolute(fvecs_np)
    median = np.median(fvecs_abs)
    desired_median = .03

    multiplication_factor = desired_median / median
    print(multiplication_factor)
    fvecs_np *= multiplication_factor
    pickle.dump(multiplication_factor, open("multifactor="+str(multiplication_factor), 'wb'))
    fvecs_np = np.clip(fvecs_np, -1, 1)

    #plt.plot(range(len(fvecs_np)), fvecs_np)  # todo for visual inspection of data
    #plt.show()
...
testX = testX.reshape(testxshape[0], 1)
...
wavenettftrain.main(finance_wave=X)
