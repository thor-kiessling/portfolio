...
def objective(variables, data=None, size=None, train=None, plot=None):
    ...
    for iteration in range(size):
        ...
        try:
            ...
            # ema1.append(ta.EMA(truncated_data[:, 4], timeperiod=variables[1]))
        ...
        #slope = abs(ema1[-1] - ema1[-2])/ema1[-1]
        if abs(mfi-50)/100 > variables[0]:
        ...
        if in_position:
            ...
            if not in_position:
                desire_position = False
        if not in_position and desire_position:  # main buy/sell todo modify this
            position_live.enter_position(entry_price=line[4])
            in_position = position_live.in_position
            trades += 1
            last_trade_time = iteration
        ...
        if in_position and not desire_position:  #  main buy/sell 2
            #                      on 20x
            position_live.exit_position(exit_price=line[4])
            returns = position_live.btc * line[4]
        ...
        if returns < 5 and not plot:  # fail-quick
            returns = 0
            # position = [0, line[1]]
            # max_draw_down = -.9271
            break
    ...
    in_position = position_live.in_position
    if in_position:
        position_live.exit_position(line[4])
        returns = position_live.btc * line[4]
        if plot:
            return_graph_x.append(line[0])
            return_graph_y.append(returns)
...
def main():
    data = extract_data()
    minutes_train = 400000  # four zeros = 1 week, five zeroes = 10 weeks,  1300000 = all my data 500000=1year
    #bounds = [(-0, 0), (2, 1000), (-0, 0), (40, 40000), (-1, 1), (10, 40000), (.0001, .002), (10, 40000)]
    #                                                                                atr reverse trigger, lookback 6/7
    #                      max            max        max         max
    bounds = [(0.0, .50), (10, 1000)]
    #           diff%,     ATR timerange
    #              0            1
    # document for bounds
    ...
    # objective([0.21078142309288023, 367.0], data=data, size=minutes_train, train=True, plot=True)  # best of MFI
    # test_result = objective([0.21078142309288023, 367.0], data=data, size=minutes_train, train=False, plot=True)
    # it resulted in
